...
/**
 * Servlet implementation class Company_Reg_Controller
 */
@WebServlet("/Company_Reg_Controller")
public class Company_Reg_Controller extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Company_Reg_Controller() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else if(S.equals("cproedit"))
		{
			Cpro_edit(request, response);
		}
	}
	
	void Cpro_edit (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		String userid = request.getParameter("id");
		System.out.println("inside Company profile " +userid);
		...
		Company_Reg_Vo uservo = new Company_Reg_Vo();
		//uservo.setUser_mst_id(mstvo);
		
		Company_Reg_Dao userdao = new Company_Reg_Dao();
		
		List userprofile = userdao.usersearch();
		session.setAttribute("usrprofile", userprofile);
		
		System.out.println("Company profile done");
		response.sendRedirect("Company/company_profile.jsp");
	}
	
	void search (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{	
		
		Company_Reg_Dao ureg = new Company_Reg_Dao();
		List urg = ureg.search1();
		
		HttpSession s = request.getSession();
		s.setAttribute("cregi", urg);
		
		response.sendRedirect("admin/manage_company.jsp");
	}
	...
	void load (HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		response.sendRedirect("Company/Company_Registration.jsp");
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		{
			response.sendRedirect(request.getContextPath()+"/Company/Company_Registration.jsp");
		}
	}
	...
	void insert(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String s1 = request.getParameter("c_name");
		String s2 = request.getParameter("c_email");
		String s3 = request.getParameter("c_number");
		String s4 = request.getParameter("c_address");
		String s5 = request.getParameter("c_username");
		String s6 = request.getParameter("c_pass");
		String s8 = request.getParameter("c_pincode");
		...
		Company_Reg_Vo v1 = new Company_Reg_Vo();
		v1.setC_name(s1);
		v1.setC_email(s2);
		v1.setC_number(s3);
		v1.setC_address(s4);
		...
		v1.setState(state);
		v1.setCity(city);
		v1.setUser_id(usrmst);
		
		
		Company_Reg_Dao d = new Company_Reg_Dao();
		d.insert(v1);
		...
		Iterator itr =  myList.iterator();
         ...
         while (itr.hasNext()) {
        	...
        	img_upld_vo.setUser_mpg_id(usrmst);
     		img_upld_vo.setUser_type("company");
	...
	protected void sendActivationLink(user_mst_vo uservo,HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		String activationLink = "http://localhost:8080/Technocraft_Debugger/Verification_Company?authentication="+linkvo.getLink()+"&proc=activation";
	...
	protected void forgotPassword(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		{		
				...
				response.sendRedirect(request.getContextPath()+"/Company/forgotpassword.jsp");
		}
	...
	protected void sendResetLink(user_mst_vo uservo,HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String activationLink = "http://localhost:8080/Technocraft_Debugger/Verification_Company?authentication="+linkvo.getLink()+"&proc=forgotPassword"; 
