        ...
        <div class="col-md-12">
          <div class="panel panel-visible">
            <div class="panel-heading">
              <div class="panel-title"> <i class="fa fa-table"></i>Manage Job</div>
            </div>
            <div class="panel-body">
              ...
              <table class="table table-striped table-bordered table-hover" id="datatable">
                <thead>
                  <tr>
                    <th>Job Id</th>
                    <th class="hidden-xs">Job Name</th>
                ...
                <tbody>
                <% int j=1; %>
                <a:forEach items="${sessionScope.list5}" var="i">
                  <tr>
                    <td><% out.print(j); j++; %></td>
                    <td class="hidden-xs"><span class="xedit">${i.job_title}</span></td>
                    <td class="hidden-xs hidden-sm" width="320px" ><span class="xedit">${i.job_description}</span></td>
                    
                    <td class="hidden-xs text-center">
                    	<div class="btn-group">
                        	<!-- <button type="button" class="btn btn-info btn-gradient"> <span class="glyphicons glyphicons-cogwheels"></span> </button> -->
                        	<button type="button" class="btn btn-success btn-gradient" onclick="fn('<%=request.getContextPath() %>/Add_job_controller?flag=edit&user=Admin&id=${i.job_id}')"> <span class="glyphicons glyphicons-cogwheels"></span> </button>
                        	<button type="button" class="btn btn-danger btn-gradient dropdown-toggle" data-toggle="dropdown" onclick="fn('<%= request.getContextPath()%>/Add_job_controller?flag=delete&user=Admin&id=${i.job_id}')"> <span class="glyphicons glyphicons-bin"></span> </button>
                      		<button type="button" class="btn btn-success btn-gradient dropdown-toggle" data-toggle="dropdown"> <span class="glyphicons glyphicons-cogwheels"></span></button>
                      		<ul class="dropdown-menu checkbox-persist pull-right text-left" role="menu">
                        	  <li><a href="<%=request.getContextPath()%>/Add_job_controller?flag=activestatus&id=${i.job_id}"><i class="fa fa-user"></i> Active </a></li>
                        	  <li><a href="<%=request.getContextPath()%>/Add_job_controller?flag=deactivestatus&id=${i.job_id}"><i class="fa fa-envelope-o"></i> Deactive </a></li>
                       	 	</ul>
                      </div>
                    
                      	<%-- <a href="<%=request.getContextPath() %>/Add_job_controller?flag=edit&id=${i.job_id}">Edit</a>/<a href="<%= request.getContextPath()%>/Add_job_controller?flag=delete&id=${i.job_id}">Delete</a> --%>
                     </td>
                  </tr>
                  </a:forEach>
                </tbody>
