        ...
        <div class="col-md-12 col-lg-6 col-lg-offset-3">
          <div class="panel">
            ...
            <div class="panel-body">
			  ...
			  <a:forEach items="${sessionScope.list}" var="a">
                <div class="form-group">
					<label for="Cat">Select Category</label>
					<select name="Cat" class="form-control">
						<a:forEach items="${sessionScope.list1}" var="j">
							<a:if test="${a.v1.cat_id==j.cat_id}">
								<option value="${j.cat_id}" selected="selected">${j.cat_name }</option>
							</a:if>
							<a:if test="${a.v1.cat_id!=j.cat_id}">
								<option value="${j.cat_id}">${j.cat_name}</option>
							</a:if>
				...
				<div class="form-group">
					...
					<select name="Sub_Cat" class="form-control">
						<a:forEach items="${sessionScope.list2}" var="k">
						<a:if test="${a.v2.sub_cat_id==k.sub_cat_id}">
							<option value="${k.sub_cat_id}" selected="selected">${k.sub_cat_name }</option>
						</a:if>
						<a:if test="${a.v2.sub_cat_id!=k.sub_cat_id }">
							<option value="${k.sub_cat_id}">${k.sub_cat_name}</option>
						</a:if>
						</a:forEach>
					</select>
                </div>
                ...
                <div class="form-group">
                  ...
                  <input id="exam_name" value="${a.exam_name}" name="exam_name" type="text" class="form-control" placeholder="Exam Name" required />
                </div>
				
                <div class="form-group">
                  <label for="exam_desc">Exam Description</label>
                  <textarea id="exam_desc" name="exam_desc" class="form-control" required >${a.exam_desc}</textarea>
                </div>
				
				<div class="form-group">
                  <label for="exam_date">Exam Date</label>
                  <%-- <input id="exam_date" value="${a.exam_date }" name="exam_date" type="date" class="form-control" placeholder="Exam Name" required /> --%>
                        <div class="input-group">
                        	  <span class="input-group-addon"><i class="fa fa-calendar"></i> </span>
                        	  <span class="input-group-addon">From</span><input type="text" name="from_exam_date" value="${a.from_exam_date}" id="datepicker" class="form-control datepicker margin-top-none" placeholder="23/9/2013" required/>
                        	  <span class="input-group-addon">To</span><input type="text" name="to_exam_date" value="${a.to_exam_date}" id="datepicker" class="form-control datepicker margin-top-none" placeholder="23/9/2013" required/>
                        </div>
                </div>
				
				<div class="form-group">
                  <label for="exam_time">Exam Time</label>
                  <%-- <input id="exam_time" value="${a.exam_time }" name="exam_time" type="text" class="form-control" placeholder="Exam Time" required /> --%>
                  		<div class="input-group"> 
                    	  	<span class="input-group-addon"><i class="glyphicons glyphicons-stopwatch "></i> </span>
                          	<input type="text" name="hours_examtime" value="${a.exam_time_hour}" class="form-control" placeholder="hours">
                          	<Span class="input-group-addon">:</Span><input type="text" name="min_examtime" value="${a.exam_time_min}" class="form-control" placeholder="min">
                        </div>
                </div>
				
				<div class="form-group">
                  <label for="total_marks">Total Marks</label>
                  <input id="total_marks" value="${a.total_marks}" name="total_marks" type="text" class="form-control" placeholder="Total Marks" required />
                </div>
				
				<div class="form-group">
                  <label for="pass_marks">Passing Marks</label>
                  <input id="pass_marks" value="${a.pass_marks }" name="pass_marks" type="text" class="form-control" placeholder="Passing Marks" required />
                </div>
                ...
                <div class="form-group">
						...
						<input class="submit btn btn-blue" type="hidden" name="id" value="${a.exam_id}">
                </div>
			</a:forEach>
              </form>
            </div>
